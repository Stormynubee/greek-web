...
async def _fetch_lockly(kind: str) -> dict:
    ...
    if r.status_code != 200:
        if cached:
            return cached[1]
        raise HTTPException(502, f"Lockly upstream: {r.status_code}")
    data = r.json()
...
async def discord_callback(code: Optional[str] = None, error: Optional[str] = None):
    ...
    if existing:
        ...
        user = User(
            ...
            role="owner" if is_owner else "viewer", points_balance=100,
        )
    ...
    resp.set_cookie(SESSION_COOKIE, token, max_age=JWT_TTL_DAYS * 86400,
...
async def admin_login(body: AdminLoginBody, request: Request, response: Response):
    ...
    response.set_cookie(ADMIN_COOKIE, token, max_age=ADMIN_TTL_HOURS * 3600,
                        httponly=True, secure=True, samesite="lax", path="/")
...
async def redeem(body: RedeemBody, user: User = Depends(_current_user)):
    ...
    if user.points_balance < reward_doc["cost"]:
        raise HTTPException(400, "Insufficient points")
    entry = await _apply_ledger(user, -reward_doc["cost"], "store_redeem",
                                ref=body.reward_id, idempotency_key=body.idempotency_key)
    if reward_doc["stock"] > 0:
        await db.rewards.update_one({"_id": oid}, {"$inc": {"stock": -1}})
    return {"ok": True, "reward": reward_doc["title"], "cost": reward_doc["cost"],
...
async def resolve_game(game_id: str, body: GameResolve, _: dict = Depends(_require_owner_or_admin)):
    ...
    winner_entries = await db.game_entries.find(winners_q).to_list(10000)
    ...
    for we in winner_entries:
        u = User.from_mongo(await db.users.find_one({"discord_id": we["user_id"]}))
    ...
    await db.games.update_one({"_id": oid}, {"$set": {
        "status": "resolved", "winning_option": body.winning_option,
...
async def join_game(body: GameJoin, user: User = Depends(_current_user)):
    ...
    if cost > 0:
        await _apply_ledger(user, -cost, "game_entry", ref=body.game_id,
...
async def admin_draw_giveaway(gid: str, _: dict = Depends(_require_owner_or_admin)):
    ...
    await db.giveaways.update_one({"_id": oid}, {"$set": {
...
async def admin_close_giveaway(gid: str, _: dict = Depends(_require_owner_or_admin)):
    from bson import ObjectId as _OID
    r = await db.giveaways.update_one({"_id": _OID(gid)}, {"$set": {"status": "closed"}})
...
async def admin_del_custom_lb(eid: str, _: dict = Depends(_require_owner_or_admin)):
    from bson import ObjectId as _OID
    await db.custom_leaderboard.delete_one({"_id": _OID(eid)})
    return {"ok": True}
...
async def admin_delete_reward(rid: str, _: dict = Depends(_require_owner_or_admin)):
    from bson import ObjectId as _OID
    await db.rewards.update_one({"_id": _OID(rid)}, {"$set": {"active": False}})
    return {"ok": True}
